...
class ExtractObsWrapper(gym.Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self.logger = TrajectoryLogger("dataset", num_envs=128, trajectory_length=100)
    ...
    def step(self, action):
        observations, _privileged_observations, rewards, dones, infos = super().step(
            action
        )
...
class AERIALRLGPUEnv(vecenv.IVecEnv):
    def __init__(self, config_name, num_actors, **kwargs):
        ...
        self.env, env_conf = env_configurations.configurations[config_name][
            "env_creator"
        ](**kwargs)
    ...
    def get_env_info(self):
        ...
        info["action_space"] = spaces.Box(
            np.ones(self.env.num_actions) * -1.0, np.ones(self.env.num_actions) * 1.0
        )
...
env_configurations.register(
    "quad",
    {
        "env_creator": lambda **kwargs: task_registry.make_env(
            "quad", args=Namespace(**kwargs)
        ),
...
env_configurations.register(
    ...
    {
        "env_creator": lambda **kwargs: task_registry.make_env(
            "quad_with_obstacles", args=Namespace(**kwargs)
        ),
        "vecenv_type": "AERIAL-RLGPU",
    },
)
...
def update_config(config, args):
    ...
    config["params"]["config"]["env_config"]["use_gpu_pipeline"] = args[
        "use_gpu_pipeline"
    ]
