...
class Controller:
    def __init__(self, control_config, device):
        ...
        self.kP = torch.tensor(
            control_config.kP, dtype=torch.float32, device=self.device
        )
        self.kV = torch.tensor(
            control_config.kV, dtype=torch.float32, device=self.device
        )
        self.kOmega = torch.tensor(
            control_config.kOmega, dtype=torch.float32, device=self.device
        )
        self.kR = torch.tensor(
            control_config.kR, dtype=torch.float32, device=self.device
        )

        self.scale_input = torch.tensor(
            control_config.scale_input, dtype=torch.float32, device=self.device
        )
        ...
        if self.control_config.controller not in control_class_dict:
            ...
            if control_class_dict[self.controller_name] is LeeAttitudeContoller:
                ...
                self.controller = LeePositionController(
                    self.kP, self.kV, self.kR, self.kOmega
                )
